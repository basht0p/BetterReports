...
export const config = { schema: schema.object({
  ...
  smtp: schema.object({ host: schema.string({ defaultValue: '' }), port: schema.number({ defaultValue: 587, min: 1, max: 65535 }), secure: schema.boolean({ defaultValue: false }), username: schema.string({ defaultValue: '' }), passwordEnv: schema.string({ defaultValue: 'BETTER_REPORTS_SMTP_PASSWORD' }), from: schema.string({ defaultValue: '' }) }),
