...
The 0.0.4 development branch passed 34 unit tests, TypeScript checks, 15 exact-platform source contracts, and the disposable 3.8.0 integration suite. Live fixtures exercised Gauge, Goal, Heat Map, Horizontal Bar, Coordinate Map, Region Map, Tag Cloud, and a saved single-series Timeline expression through actual authorized aggregation queries and Letter PDF output. The optional Maps application plugin was absent from that Docker fixture, so its standalone saved-object adapter has fixture validation only.
