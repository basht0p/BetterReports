...
Replace both plugin ZIPs using the same stop, remove, install, and restart procedure above. The new visualization adapters do not change the report storage schema or worker permissions. Existing reports, schedules, and grants remain stored. A report must use **Refresh sources** to adopt changes to an imported visualization or map; refreshing creates a new revision and requires scheduled authorization again. The Maps source entry appears only when the optional OpenSearch Dashboards Maps plugin is installed. See [Support](SUPPORT.md) for the supported Maps and Timeline subsets and the visualizations that remain excluded.
